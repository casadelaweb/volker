<template>
  ...
  <section class="product">
      ...
      <div class="product-container">
          ...
          <div class="sliders-sticky">
            <div class="slider_bg">
              ...
              <template v-if="product.images && product.images.length > 1">
                <swiper :lazy="settings.lazy" :loop="settings.loop" :modules="modules"
                  ...
                  <swiper-slide v-for="(img, index) in product.images" :key="index">
                    <img :alt="product.name" :data-src="img.url" class="swiper-lazy slider-img"
              ...
              <template v-else-if="product.images && product.images.length === 1">
                <img :alt="product.name" :src="product.images[0].url" class="slider-img">
              </template>
              <template v-else>
                <img :alt="product.name" class="slider-img" src="src/assets/img/placeholder.jpg">
              </template>
            </div>
            <div class="slider-thumbs">
              <div v-for="(img, index) in product.images" :key="index" class="slider-thumb">
                <img :alt="product.name" :data-src="img.thumbSmall.url" class="swiper-lazy"
                     src="src/assets/img/placeholder.jpg">
                <div class="swiper-lazy-preloader"></div>
              </div>
            </div>
        ...
        <div class="product-information">
          ...
          <div v-if="product.parameters.length > 0" class="product-parameters">
            <div v-for="(parameter, index) in product.parameters" :key="index" class="product-parameter">
              <div class="product-parameter-name">{{ parameter.name }}</div>
          ...
          <div class="calculator">
            <div class="calculator-top">
              <label class="form-group">
                ...
                <input class="form-input" max="999" min="0" placeholder="Сколько заказать?" type="number">
              </label>
            </div>
            <div class="calculator-discounts">
              ...
              <label class="calculator-label" title="Скидка 3% при покупке от 150 ед. и 5% при покупке от 300 ед.">
                <input class="calculator-checkbox" data-alt type="checkbox">
                <span class="calculator-label-text"> Оптовая скидка </span>
              </label>
              <div class="calculator-submit-info">
                Оптовая скидка применяется автоматически - 3% при покупке от 150 ед. и 5% при покупке от 300 ед.
              </div>
            </div>
            <div class="calculator-bottom">
              ...
              <div class="calculator-submit-info">Указанная информация носит ознакомительный характер и
                не является публичной офертой. Наличие и актуальную стоимость узнавайте у продавца.
              </div>
              <router-link class="calculator-submit-button" title="Перейти в контакты" to="/contacts">
                В контакты
              </router-link>
            </div>
          </div>
    ...
    <img v-else alt="loading" class="lazy" data-src="src/assets/img/loading.gif" src="src/assets/img/placeholder.jpg">
  </section>
  ...
  <suggestions :products="suggestions.products" :title="suggestions.title" />
</template>
