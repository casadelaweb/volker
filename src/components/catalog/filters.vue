  ...
  <div class="filters">
    <div class="filters-content">
      <div class="filter-group">
        <input v-model="filters.searchQuery" class="filters-search" placeholder="Искать что-нибудь" type="text"
      ...
      <template v-for="filterGroup in filters" :key="filterGroup.inputName">
        <div v-if="filterGroup.list && filterGroup.list.length > 0" class="filter-group">
          ...
          <label v-for="filter in filterGroup.list" :key="filter.inputValue" class="filter-label">
            <input v-model="filterGroup.model" :name="filterGroup.inputName" :type="filterGroup.inputType"
                   :value="filter.inputValue" class="filter-checkbox" @change="filterProductsByCategory">
      ...
      <div class="filter-group filter-group_suggest">
        ...
        <button data-modal-open="modal-suggest-filter" type="button">Подскажите что нужно добавить</button>
      </div>
    </div>
  </div>
